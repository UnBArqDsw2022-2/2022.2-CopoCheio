...
interface DrinksModalBodyInterface {
    ...
    setDrinkInfoObject: Dispatch<SetStateAction<{ title: string; userName: string; time?: number; base: string[]; dificulty?: string | null; country?: string; ingredients: string; guide: string; }>>;
}
...
const DrinksModalBody = ({
    ...
    return (
        <BodyContainer>
            ...
            <CardSide>
                {drinkInfoObject.image ? <Image src={drinkInfoObject.image} height={'40%'} width={'100%'} borderRadius={'8px'} marginRight='4px' /> :
                    <BlankImage>
                ...
                <IconRow>
                    <LeftIcon onClick={() => handleSetOnClickInput('dificulty')}>
                        <IconText iconColor={colors.primary} iconLeft='school' iconSize='18px' fontColor={colors.grey} fontSize='16px' children={drinkInfoObject.dificulty || undefined} />
                        {
                            onClickInput === 'dificulty' && <DropDown element={
                                <>
                                    ...
                                    <Text onClick={() => handleDificulty('Díficil')} margin="8px 0 0 0" weight="regular" size="14px" color={colors.grey} >Difícil</Text>
                                </>
                            } />
                        }
                    </LeftIcon>
